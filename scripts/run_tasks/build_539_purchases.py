...
def read_purchase_rows(results_by_draw_no: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    with CSV_PATH.open(newline="", encoding="utf-8") as file_obj:
        ...
        for index, raw_row in enumerate(reader):
            ...
            number_labels = [ball_label(number) for number in numbers]
...
def ball_items(values: list[str], row: dict[str, Any]) -> str:
...
def build_result_note(rows: list[dict[str, Any]]) -> list[str]:
    results = known_results(rows)
    if not results:
        return []
...
def build_markdown(rows: list[dict[str, Any]], summary: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "---",
        "",
        "<!-- 此檔由 scripts/run_tasks/build_539_purchases.py 自動產生，請勿手改；如需修改請改 raw-data/lotto-purchases/539-purchases.csv 後執行 python scripts/run.py -->",
        "",
    ]
    ...
    lines.extend(
        [
            (
                f"從 {summary['date_start']} 開始記錄，總計 {summary['total_records']:,} 注，"
                f"總計花費 {summary['total_spent']:,} 元，已結算 {summary['settled_records']:,} 筆，"
                f"已結算中獎 {summary['known_prize']:,} 元，已結算淨額 {summary['known_net']:,} 元。"
            ),
            "",
        ]
    )
    ...
    lines.extend(build_result_note(rows))
