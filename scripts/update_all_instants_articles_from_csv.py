...
def load_rows_by_game() -> dict[str, list[dict[str, str | int | datetime]]]:
    rows_by_game: dict[str, list[dict[str, str | int | datetime]]] = defaultdict(list)
            ...
            rows_by_game[game].append(
                {
                    ...
                    "prize": int((row.get("prize") or "0").strip()),
                }
            )
...
def build_manual_test_section(rows: list[dict[str, str | int | datetime]], newline: str) -> str:
    lines = ["## 親自實測", ""]
    for row in rows:
...
def update_article(path: Path, rows_by_game: dict[str, list[dict[str, str | int | datetime]]]) -> bool:
