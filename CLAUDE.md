...
**Read `AGENTS.md` first** — it defines the branch/PR conventions (branch per task from `main`, draft PRs by default), commit style (separate manual data edits, generated output, and script changes into distinct commits), and the step-by-step workflows for each kind of data update. One override: the `codex/{description}` branch prefix in AGENTS.md is reserved for Codex — Claude Code branches use `claude/{description}` so the user can tell which agent did the work.
...
# Refresh official draw results from Taiwan Lottery (monthly-ish; the sync step
# is REQUIRED before run.py — run.py reads research/derived/, not the downloads)
...
Data flows one way: `raw-data/` (sources of truth) → `scripts/run.py` → generated files in `docs/`. Draw results take an extra hop: `raw-data/lotto-result-downloads/` → `sync_lotto_research_data.py` → `research/derived/{game}_all_years.csv`, which is what the run.py tasks (stats, purchases, latest draws) actually read.
...
- `raw-data/lotto-purchases/{539,638,649}-purchases.csv` — draw-game purchase records; only purchase basics. Prize rank/amount are *computed* by `build_{game}_purchases.py` from draw results (fixed-prize rules documented in `scripts/README.md`; jackpot/variable prizes show as pending).
...
- `research/` — `research/derived/` and `research/by-game/` are generated by `sync_lotto_research_data.py` and feed run.py (see above); `research/638/` is offline prediction research (backtesting models), not part of the site build.

**Generated — do not hand-edit** (regenerate via `python scripts/run.py` instead): `docs/_list/instants-all.md`, `docs/_list/{539,638,649}-purchases.md`, `docs/assets/data/*.json`, `docs/_data/latest_draws.json`. `docs/_site/` is Jekyll build output. Note that `update_all_instants_articles_from_csv.py` also rewrites the manual-record sections of instants articles from the CSV.
